...
public class MimeUtil {
    private static final Logger logger =
            Logger.getLogger(MimeUtil.class);
	...
	static {
		...
		try {
			...
			if (!found) {
				...
				is = MimeUtil.class.getClassLoader().getResourceAsStream("it/jugpadova/util/mime/magic.mime");
			}
			parse(new InputStreamReader(is));
		}catch(Exception e) {
			e.printStackTrace();
    ...
    private static String getMagicMimeType(byte[] content) throws IOException {
        int len = mMagicMimeEntries.size();
        for (int i=0; i < len; i++) {
            MagicMimeEntry me = (MagicMimeEntry) mMagicMimeEntries.get(i);
